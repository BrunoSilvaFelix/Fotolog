...
class BinaryTree(TreeADT):
    ...
    def __len__(self):
        return len(self.__in_order(self._root))
    ...
    def brothers(self, v1, v2):
        v1_exists, node1, depth_v1 = self.search(v1)
        v2_exists, node2, depth_v2 = self.search(v2)
        result = False
        if node1 != node2:
            pass
    ...
    def cousins(self, v1, v2):
        ...
        v2_exists, node2, depth_v2 = self.search(v2)
        result = False
        if v1_exists and v2_exists and depth_v1 == depth_v2:
            pai_n1 = node1.parent_node()
            pai_n2 = node2.parent_node()
            if pai_n1 != pai_n2 and \
                    pai_n1.parent_node() == pai_n2.parent_node():
                result = True
        return result

    def mirror(self):
        my_nodes = self.__pre_order(self.root(), [])
    ...
    def longest_path_values(self):
        ...
        return result.reverse()  # caminho inverso!

    def in_order(self):
        stack = []
        result = []
        visited_nodes = self.__qtd_nodes
        if self._root:
            node = self._root
            stack.append(node)
